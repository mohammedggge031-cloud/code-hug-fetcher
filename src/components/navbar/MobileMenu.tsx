              ...
              return (
                <div key={l.en} className="border-b border-primary-foreground/5 last:border-0">
                  ...
                  {hasDropdown && isExpanded && (
                    <div className="ps-3 md:ps-4 pb-3 border-s-2 border-accent/30 ms-2">
                        ...
                        return (
                          <div key={i}>
                              ...
                              {hasSubItems && item.isRoute ? (
                                <>
                                  <Link
                                    to={item.href}
                                    onClick={closeMobileMenu}
                                    className="flex-1 flex items-center gap-2.5 md:gap-3 px-3 py-2.5 md:py-3 text-sm md:text-base font-medium text-primary-foreground/75 hover:text-accent transition-colors rounded-lg hover:bg-primary-foreground/5"
                                  >
                                  ...
                                  <button
                                    ...
                                    className="inline-flex h-9 w-9 md:h-10 md:w-10 items-center justify-center rounded-lg text-primary-foreground/60 transition-colors hover:bg-primary-foreground/5 hover:text-accent"
                                    aria-label={t(`Expand ${item.labelEn}`, `افتح ${item.labelAr}`)}
                                  >
                                    <ChevronDown className={`w-3.5 h-3.5 md:w-4 md:h-4 transition-transform duration-300 ${isSubExpanded ? "rotate-180" : ""}`} />
                                  </button>
                                </>
                              ) : item.isRoute ? (
                                <Link
                            ...
                            {hasSubItems && isSubExpanded && (
                              <div className="ps-9 md:ps-10 pb-1">
                                {item.subItems!.map((sub, j) =>
                                  sub.isRoute ? (
                                    <Link
                                      ...
                                      to={sub.href}
                                      onClick={closeMobileMenu}
                                      className="block px-3 py-2 md:py-2.5 text-xs md:text-sm font-medium text-primary-foreground/50 hover:text-accent transition-colors"
                                    >
