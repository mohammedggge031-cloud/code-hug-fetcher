...
const Navbar = () => {
  ...
  return (
    <>
    <header className={`site-fixed-layer touch-fixed-ui fixed left-0 right-0 top-0 z-50 lg:transition-[box-shadow] lg:duration-300 ${scrolled ? "lg:shadow-elevated" : ""}`}>
      <div className="bg-primary shadow-none lg:shadow-sm">
          ...
          <div className="flex lg:hidden items-center justify-between w-full gap-2">
            ...
            {/* Center: hamburger */}
              ...
              onClick={() => { cancelPendingScroll(); setMobileOpen(!mobileOpen); }}
                className="w-10 h-10 rounded-xl flex items-center justify-center text-primary-foreground transition-none hover:bg-primary-foreground/10 lg:transition-colors"
              aria-label="Toggle menu"
            >
              <div className="relative w-5 h-5 flex items-center justify-center">
                  <span className={`absolute h-[2px] w-5 bg-primary-foreground rounded-full transition-none lg:transition-all lg:duration-300 ${mobileOpen ? "rotate-45" : "-translate-y-1.5"}`} />
                  <span className={`absolute h-[2px] w-5 bg-primary-foreground rounded-full transition-none lg:transition-all lg:duration-300 ${mobileOpen ? "opacity-0 scale-0" : "opacity-100"}`} />
                  <span className={`absolute h-[2px] w-5 bg-primary-foreground rounded-full transition-none lg:transition-all lg:duration-300 ${mobileOpen ? "-rotate-45" : "translate-y-1.5"}`} />
              </div>
            </button>
            ...
            <div className="flex items-center gap-1.5">
              <a
                ...
                className="flex items-center whitespace-nowrap rounded-xl bg-accent px-4 py-2.5 text-xs font-bold uppercase tracking-wide text-accent-foreground shadow-soft transition-none lg:shadow-elevated lg:transition-[filter] lg:duration-200 lg:hover:brightness-110"
              >
    ...
    <div className={`lg:hidden fixed inset-x-0 bottom-0 top-16 z-[55] overscroll-none bg-primary transition-[opacity,visibility] duration-300 ${mobileOpen ? "opacity-100 visible" : "opacity-0 invisible pointer-events-none"}`}>
