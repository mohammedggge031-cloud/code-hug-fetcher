...
const CHALLENGES: Challenge[] = [
  ...
  { problemEn: "\"I live in a very different timezone from Egypt.\"", problemAr: "\"أعيش في منطقة زمنية بعيدة عن مصر.\"", solutionEn: "Classes are one-on-one and scheduled flexibly. Students from the USA, Canada, UK, Australia and Europe pick slots that fit their day.", solutionAr: "الحصص فردية وتُحجز على مدار الساعة. طلابنا من أمريكا وكندا وبريطانيا وأستراليا وأوروبا يختارون المواعيد التي تناسبهم — قبل الفجر أو بعد العشاء أو في العطلات." },
];
...
const CURRICULUM: CurriculumWeek[] = [
  ...
  { weekEn: "Phase 2 — Ijazah Khatmah Begins", weekAr: "المرحلة 2 — بدء ختمة الإجازة", topicEn: "Continuous Talaqqi from Al-Fatiha", topicAr: "تلقٍّ متصل من الفاتحة", detailsEn: ["Formal Khatmah opens under the shaykh's ear from Surah Al-Fatiha", "Every page recited to the shaykh until fully corrected", "Written progress notes and error log kept for every session", "A steady steady rhythm the shaykh sets with the student"], detailsAr: ["افتتاح الختمة الرسمية على الشيخ من سورة الفاتحة", "كل صفحة تُتلى على الشيخ حتى تُصحَّح كليًا", "ملاحظات تقدم مكتوبة وسجل للأخطاء في كل حصة", "وتيرة أسبوعية منتظمة يضعها الشيخ مع الطالب"] },
...
const FAQS: FAQ[] = [
  ...
  { questionEn: "How long will the Ijazah take?", questionAr: "كم يستغرق الحصول على الإجازة؟", answerEn: "Your shaykh sets a personalized pace after the free assessment based on your current Tajweed level, Hifz strength and availability. Progress is tracked page by page so you always know where you are on the Khatmah.", answerAr: "يضع الشيخ خطة زمنية مخصصة بعد حصة التقييم المجانية بحسب مستواك في التجويد وقوة حفظك وجدولك الأسبوعي. يُتابَع التقدم صفحة صفحة فتعرف دائمًا موقعك من الختمة." },
...
const IjazahProgramCourse = () => (
  <ServicePageLayout
    ...
    aboutContentEn={[
      ...
      "We serve Ijazah students from the USA, Canada, UK, Australia, Germany, France and across the Muslim world. Classes are scheduled flexibly to fit any timezone, and we offer full brother-with-shaykh and sister-with-shaykhah tracks so every student is comfortable throughout the Khatmah.",
      "The path is clear: a free assessment, then either the pre-Ijazah polishing phase or the direct opening of the Ijazah Khatmah, followed by steady regular Talaqqi until the Khatmah is sealed and the written Ijazah is issued with the full Sanad chain and your name added at its end.",
    ]}
    aboutContentAr={[
    ...
    methodContentEn={[
      ...
      "Progress is tracked in a private student portal: the Khatmah tracker shows every page you have recited and cleared, written notes from the shaykh document your strengths and remaining issues, and you always know exactly how far you are from Ijazah.",
    ...
    featuresEn={[
      ...
      "Progress-tracking portal and Khatmah progress reports",
    ]}
    featuresAr={[
    ...
    deepContentEn={[
      ...
      "The typical journey is: free assessment → short pre-Ijazah polishing (if needed) → opening of the Ijazah Khatmah → steady regular Talaqqi at a pace your shaykh sets with you → oral exam → written Ijazah with the full Sanad. Some students pursue Ijazah in Tajweed only, some in Hifz only, and some complete both.",
    ...
    deepContentAr={[
      ...
      "المسار المعتاد: حصة تقييم مجانية → مرحلة صقل ما قبل الإجازة عند الحاجة → افتتاح ختمة الإجازة → تلقٍّ أسبوعي منتظم بوتيرة يضعها الشيخ معك → امتحان شفهي → إجازة مكتوبة بالسند الكامل. بعض الطلاب يأخذ إجازة التجويد فقط، والبعض إجازة الحفظ فقط، والبعض يجمع بينهما.",
